// Data khodam yang sudah ditentukan
const khodamData = [
...
// Mendengarkan submit form
document.getElementById('khodamForm').addEventListener('submit', function(event) {
    event.preventDefault();
    const name = document.getElementById('name').value.trim();
    const result = document.getElementById('result');

    // Memilih secara acak khodam dari data
    const randomKhodam = khodamData[Math.floor(Math.random() * khodamData.length)];

    // Menampilkan hasil khodam
    result.innerHTML = `
        <hr class="my-4 border-black w-full">
        <div class="my-4">
            <p class="text-lg font-semibold text-gray-800 text-center">${name}, Khodam Anda:</p>
            <p class="text-2xl font-bold text-green-500 text-center">${randomKhodam.name}</p>
            <p class="text-sm text-gray-600 text-center">${randomKhodam.meaning}</p>
        </div>
    `;
});
